...
void onPacketReceived() {
    ...
    case SERIAL_PACKET_TYPE_RSSI_REQUEST:
    {
        ...
        auto rssiRequestPacket = (SerialPacketRSSI_Request_t*)floodPacket->payload;

        auto payloadBuffer = (uint8_t*)malloc(sizeof(SerialPacketRSSI_Response_t));
        ((SerialPacketRSSI_Response_t*)payloadBuffer)->nodeID = rssiRequestPacket->nodeID;
        ((SerialPacketRSSI_Response_t*)payloadBuffer)->nodeRSSI = getMeshRouter()->getRSSI(rssiRequestPacket->nodeID);

        //reuse Serial Header for response
        serialPacketHeader->serialPacketType = SERIAL_PACKET_TYPE_RSSI_RESPONSE;
        serialPacketHeader->size = sizeof(SerialPacketRSSI_Response_t);
        //send message
        {
    ...
    case SERIAL_PACKET_TYPE_SNR_REQUEST:
    {
        ...
        //reuse Serial Header for response
        serialPacketHeader->serialPacketType = SERIAL_PACKET_TYPE_SNR_RESPONSE;
        serialPacketHeader->size = sizeof(SerialPacketSNR_Response_t);
    ...
    if (availableBytes) {
        ...
        if (serialStatus == SERIAL_REC_PAYLOAD && availableBytes) {
            ...
            uint16_t bytesToRead =
                availableBytes + serialIndex > serialPacketHeader->size ?
                serialPacketHeader->size - serialIndex : availableBytes;
