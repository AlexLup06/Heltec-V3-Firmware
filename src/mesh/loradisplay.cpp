...
void LoraDisplay::printRoutingTableScreen(RoutingTable_t **routingTable, uint8_t totalRoutes, uint8_t nodeID, double_t update)
{
    display.clearDisplay();
    ...
    {
        char macHexStr[8] = {0};
        sprintf(macHexStr, "%02X%02X%02X", routingTable[i]->deviceMac[3], routingTable[i]->deviceMac[4], routingTable[i]->deviceMac[5]);
    ...
    drawInfoFooter(nodeID, update);

    display.display();
}

void LoraDisplay::drawInfoFooter(uint8_t nodeID, double_t update)
{
